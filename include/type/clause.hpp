...
public:
    ...
    void calc_abstraction(void) {
        ...
        for (int i = 0; i < size(); i++) {
            abstraction |= 1 << (_data[i].lit.variable() & 31);
        }
...
public:
    ...
    ClauseAllocator(uint32_t start_cap) : _ra(start_cap), extra_clause_field(false) {}

    ClauseAllocator(void) : extra_clause_field(false) {}
