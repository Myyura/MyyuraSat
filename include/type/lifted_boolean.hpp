...
public:
    ...
    bool operator==(const LiftedBoolean& lb) const {
        return ((lb._value & 2) & (_value & 2)) | (!(lb._value & 2) & (_value == lb._value));
    }
    bool operator!=(const LiftedBoolean& lb) const { return !(*this == lb); }
    LiftedBoolean operator^(bool b) const { 
        return LiftedBoolean((uint8_t)(_value ^ (uint8_t)b));
    }

    LiftedBoolean operator&&(const LiftedBoolean& lb) const {
    ...
    int to_int(void) const { return _value; }
};
